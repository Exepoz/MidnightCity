...
local function takeCoral(coral, obj)
    ...
    QBCore.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('qb-diving:server:TakeCoral', currentDivingLocation.area, coral, true, false)
        ClearPedTasks(ped)
...
local function TakeBox(current, obj)
    if Config.CoralLocations[currentDivingLocation.area].coords.Box[current].PickedUp then return end
    local ped = PlayerPedId()
    local times = math.random(2, 5)
    FreezeEntityPosition(ped, true)
    QBCore.Functions.Progressbar("take_box", "Collecting Box", times * 1000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        Config.CoralLocations[currentDivingLocation.area].coords.Box[current].PickedUp = true
        TriggerServerEvent('qb-diving:server:TakeCoral', currentDivingLocation.area, current, true, true)
        ClearPedTasks(ped)
        FreezeEntityPosition(ped, false)
        DeleteEntity(obj)
    end, function() -- Cancel
...
local function setDivingLocation(divingLocation)
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            local model = `prop_coral_pillar_01`
            RequestModel(model)
            while not HasModelLoaded(model) do
                Wait(0)
            end
            local object = CreateObject(model, v.coords.x, v.coords.y, v.coords.z, false, false, false)
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Box) do
        if Config.UseTarget then
            local model = `prop_lev_crate_01`
            RequestModel(model)
            while not HasModelLoaded(model) do
                Wait(0)
            end
...
local function CreateGroupPed()
    for k, v in pairs(Config.GroupPedLocations) do
        local current = Config.GroupPedLocations[k]
        local model = 'a_m_m_salton_01'
        RequestModel(model)
        while not HasModelLoaded(model) do
            Wait(0)
        end
        local currentCoords = vector4(current.x, current.y, current.z - 1, current.w)
        ...
        exports['qb-target']:AddTargetEntity(ped,{
            options = {
                ...
                {
                    type = 'client',
                    event = 'qb-diving:client:Start',
                    action = function()
                        TriggerEvent('qb-diving:client:Start', k)
                    end,
                    icon = 'fas fa-fish',
                    label = 'Start Solo!',
                },
...
RegisterNetEvent('qb-diving:client:Start', function(getspawn)
    ...
    TriggerServerEvent('qb-vehiclekeys:server:AcquireVehicleKeys', GetVehicleNumberPlateText(boat))
