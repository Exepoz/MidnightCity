...
Padlock.Close = function() {
    ...
    $.post('https://cr-fleecabankrobbery/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://cr-fleecabankrobbery/PadLockClose');
    // if (data.error != null) {
    //     $.post('https://cr-fleecabankrobbery/CombinationFail');
    // }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://cr-fleecabankrobbery/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://cr-fleecabankrobbery/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://cr-fleecabankrobbery/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            if (CurrentType == "keypad") {
                ...
                $.post('https://cr-fleecabankrobbery/exit');
            }
        }
    };
}); //docready
...
function outOfPins() {
    gameOver = true;
    $.post('https://cr-fleecabankrobbery/fail');
    setTimeout(function(){
        reset()
    }, 250)
}
...
function unlock() {
    gameOver = true;
    $.post('https://cr-fleecabankrobbery/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
