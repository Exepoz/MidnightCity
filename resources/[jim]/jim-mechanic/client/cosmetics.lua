...
local function getMechBoardOption(plate, part)
    local pData = QBCore.Functions.GetPlayerData()
    local d = {}
    for k, v in pairs(pData.items) do
        if v.name == 'mechboard' and string.find(v.info.vehplate, plate) then
            for k_, v_ in pairs(v.info.modlist) do
                if v_.i == part and cosmeticTable[v_.i] then
                    for a, b in pairs(cosmeticTable[v_.i]) do
                        if b.id == v_.mod or b.window then
                            print("adding ", v_.chosenMod, part, b.id, b.header, v_.chosenTint)
                            d[#d+1] = {mod = v_.chosenMod, part = part, id = b.id, lbl = b.header, tint = v_.chosenTint}
                        end
                    end
                end
            end
...
RegisterNetEvent('jim-mechanic:client:Cosmetic:Check', function(part) local validMods, Menu, vehicle, Ped = {}, {}, nil, PlayerPedId()
	...
	if DoesEntityExist(vehicle) then
        if QBCore.Functions.HasItem('mechboard') then mechBoard = getMechBoardOption(trim(GetVehicleNumberPlateText(vehicle)), part) end
        if mechBoard then
            for k, mBOptions in pairs(mechBoard) do
                if mBOptions.tint then
                    local txt, disabled, icon, name, iText = "", false, "", "Stock.", " - Select to install."
                    for l, b in pairs(Loc[Config.Lan].vehicleWindowOptions) do
                        if mBOptions.tint == b.id then name = b.name end
                        if GetVehicleWindowTint(vehicle) == b.id and mBOptions.tint == b.id then name = b.name txt = Loc[Config.Lan]["common"].current disabled = true icon = "fas fa-check" iText = " - Already Installed." end
                    end
                    validMods = { mod = mBOptions.tint, name = name, part = part, window = true, install = txt, disabled = disabled, icon = icon }
                    Menu[#Menu+1] = {
                        icon = icon, isMenuHeader = disabled,
                        header = "Detected Mechboard Choice!",
                        txt = "Window Tint".." - "..name..iText,
                        onSelect = function() TriggerEvent("jim-mechanic:client:Cosmetic:Apply", validMods) end,
                    }
                else
                    local txt, disabled, icon, iText = "", false, "", " - Select to install."
                    for i = 1, GetNumVehicleMods(vehicle, mBOptions.id) do
                        if GetVehicleMod(vehicle, mBOptions.id) == mBOptions.mod then txt = Loc[Config.Lan]["common"].current disabled = true icon = "fas fa-check" iText = " - Already Installed." end
                    end
                    validMods = { mod = mBOptions.mod, id = mBOptions.id, name = GetLabelText(GetModTextLabel(vehicle, mBOptions.id, mBOptions.mod)), part = mBOptions.part, install = txt, disabled = disabled, icon = icon }
                    Menu[#Menu+1] = {
                        icon = icon, isMenuHeader = disabled,
                        header = "Detected Mechboard Choice!",
                        txt = mBOptions.lbl.." - "..GetLabelText(GetModTextLabel(vehicle, mBOptions.id, mBOptions.mod))..iText,
                        onSelect = function() TriggerEvent("jim-mechanic:client:Cosmetic:Apply", validMods) end,
                    }
                end
            end
        ...
        if countTable(Menu) ~= 0 then
            openMenu(Menu, { header = searchCar(vehicle), headertxt = Loc[Config.Lan]["check"].label10, canClose = true, onExit = function() end, })
        else
	...
	if DoesEntityExist(vehicle) then
        ...
        if not data.horn and not data.plate and not data.extra then
            Menu[#Menu+1] = {
                ...
                onSelect = function()
                    TriggerEvent("jim-mechanic:client:Cosmetic:Apply", { mod = -1, id = data.id, part = data.part, plate = data.plate, window = data.window, oldLiv = data.oldLiv, roofLiv = data.roofLiv, extra = data.extra })
                end,
            }
        end
        if data.horn then
		...
		for l, b in pairs(validMods) do
			Menu[#Menu+1] = {
                ...
                onSelect = function() TriggerEvent("jim-mechanic:client:Cosmetic:Apply", b) end,
			}
		end
        openMenu(Menu, {
            header = searchCar(vehicle),
            headertxt = data.header..br..(isOx() and br or "")..Loc[Config.Lan]["common"].amountoption..countTable(validMods),
            onBack = function() TriggerEvent("jim-mechanic:client:Cosmetic:Check", data.part) end,
        })
	end
end)
...
RegisterNetEvent('jim-mechanic:client:Cosmetic:Apply', function(data) local Ped = PlayerPedId()
	...
	if modName == "NULL" or (data.plate or data.oldLiv or data.roof) then modName = Loc[Config.Lan]["exterior"].stockMod end
    if data.window then modName = "new Window Tint..." end
