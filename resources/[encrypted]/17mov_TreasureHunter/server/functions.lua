...
function Pay(source, amount, itemsToGive)
    if Config.Framework == "QBCore" then
            ...
            if itemsToGive > 0 then
                for _, item in pairs(Config.RewardItemsToGive) do
                    if math.random(100) <= item.chance then
                        if math.floor(itemsToGive * item.amountPerTreasure) >= 1 then
                            Player.Functions.AddItem(item.item_name, math.floor(itemsToGive * item.amountPerTreasure))
                        end
                    end
                end
            end
        ...
        if xPlayer ~= nil and xPlayer.addMoney ~= nil then
            ...
            if itemsToGive > 0 then
                for _, item in pairs(Config.RewardItemsToGive) do
                    if math.random(100) <= item.chance then
                        if math.floor(itemsToGive * item.amountPerTreasure) >= 1 then
                            xPlayer.addInventoryItem(item.item_name, math.floor(itemsToGive * item.amountPerTreasure))
                        end
                    end
                end
            end
...
function isHaveRequiredItem(source)
    if Config.RequiredItem ~= "none" then
        if Config.Framework == "QBCore" then
            ...
            return counter > 0
        elseif Config.Framework == "ESX" then
            if Core.GetPlayerFromId(source).getInventoryItem(Config.RequiredItem).count > 0 then
                return true 
            else
                return false
            end
