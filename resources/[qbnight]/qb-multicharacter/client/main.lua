...
-- Functions
local stopWChange = false
local function skyCam(bool)
    if bool then
        stopWChange = false
        StartAudioScene("CHARACTER_CHANGE_IN_SKY_SCENE")
        TriggerEvent('qb-weathersync:client:DisableSync',  Config.Weather.time, Config.Weather.weather)
        TriggerServerEvent('qb-multicharacter:server:CharBucket')
        Wait(500)
        ...
        Citizen.CreateThread(function()
            Wait(52000)
            if stopWChange then return end
            TriggerEvent('qb-weathersync:client:DisableSync',  Config.Weather.time, 'CLEARING', true)
            Wait(4000)
            UpdateAnim()
            Wait(16000)
            if stopWChange then return end
            TriggerEvent('qb-weathersync:client:DisableSync',  Config.Weather.time, 'EXTRASUNNY', true)
        end)
    else
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    Wait(500)
    TriggerServerEvent('qb-multicharacter:server:WorldBucket')
    SendNUIMessage({
        action = "stopMusic"
    })
end)
...
function UpdateAnim()
    ...
    RandomAnims = anims
    if not charPed then return end
    SetPedShouldPlayNormalScenarioExit(charPed)
    Wait(9000)
    SetPedShouldPlayImmediateScenarioExit(charPed)
