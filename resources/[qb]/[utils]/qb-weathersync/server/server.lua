...
local function nextWeatherStage()
    ...
    elseif CurrentWeather == "CLEARING" or CurrentWeather == "OVERCAST" then
        ...
        elseif new == 3 then CurrentWeather = (CurrentWeather == "CLEARING") and "FOGGY" or "RAIN" --CurrentWeather = "CLEAR"
        elseif new == 4 then CurrentWeather = (CurrentWeather == "RAIN") and "THUNDER" or "RAIN" --CurrentWeather = "EXTRASUNNY"
...
QBCore.Commands.Add('settime', Lang:t('help.timecommand'), {{ name=Lang:t('help.timehname'), help=Lang:t('help.timeh') }, { name=Lang:t('help.timemname'), help=Lang:t('help.timem') }}, true, function(source, args)
...
--     while true do
--         Wait(0)
--         local newBaseTime = os.time(os.date("!*t")) / 4 + 360 --Set the server time depending of OS time 9:23 - 9:38  (15) ////// 17:13 17:44
--         if (newBaseTime % 60) ~= previous then --Check if a new minute is passed
--             previous = newBaseTime % 60 --Only update time with plain minutes, seconds are handled in the client
--             if freezeTime then
--                 timeOffset = timeOffset + baseTime - newBaseTime
