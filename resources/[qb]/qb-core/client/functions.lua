...
function QBCore.Functions.Notify(text, texttype, length)
    ...
    if type(text) == "table" then
        ...
        lib.notify({title = caption, description = ttext, type = texttype, duration = length, position = 'center-right'})
    else
        texttype = texttype or 'inform'
        length = length or 5000
        lib.notify({title = '', description = text, type = texttype, duration = length, position = 'center-right'})
    end
end
