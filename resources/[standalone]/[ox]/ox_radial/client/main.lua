...
local function SetupRadialMenu()
    ...
    if Config.GangInteractions[QBCore.Functions.GetPlayerData().gang.name] then
        lib.addRadialItem(convert({
            id = 'ganginteractions',
            label = Lang:t("general.gang_radial"),
            icon = 'skull-crossbones',
            items = Config.GangInteractions[QBCore.Functions.GetPlayerData().gang.name]
        }))
    end

    if not QBCore.Functions.GetPlayerData().job.onduty then return end
    if not Config.JobInteractions[QBCore.Functions.GetPlayerData().job.name] then return end

    lib.addRadialItem(convert({
        id = 'jobinteractions',
        label = Lang:t("general.job_radial"),
        icon = 'briefcase',
        items = Config.JobInteractions[QBCore.Functions.GetPlayerData().job.name]
    }))
end

local function IsPolice()
    return QBCore.Functions.GetPlayerData().job.type == "leo" and QBCore.Functions.GetPlayerData().job.onduty
end

local function IsEMS()
    return QBCore.Functions.GetPlayerData().job.type == "medic" and QBCore.Functions.GetPlayerData().job.onduty
end
...
RegisterNetEvent('radialmenu:flipVehicle', function()
    ...
    lib.progressBar({
        ...
        if distance <= 15 then
            ...
            QBCore.Functions.Notify(Lang:t("success.flipped_car"), 'success')
        else
...
RegisterNetEvent('QBCore:Client:SetDuty', function(onduty)
    lib.removeRadialItem('jobinteractions')
    if onduty and Config.JobInteractions[QBCore.Functions.GetPlayerData().job.name] then
        lib.addRadialItem(convert({
            id = 'jobinteractions',
            label = Lang:t("general.job_radial"),
            icon = 'briefcase',
