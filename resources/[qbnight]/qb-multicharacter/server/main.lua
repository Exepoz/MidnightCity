...
RegisterNetEvent('qb-multicharacter:server:CharBucket', function()
    SetPlayerRoutingBucket(source, source+1)
end)

RegisterNetEvent('qb-multicharacter:server:WorldBucket', function()
...
RegisterNetEvent('qb-multicharacter:server:loadUserData', function(cData)
    ...
    if QBCore.Player.Login(src, cData.citizenid) then
        ...
        print("^2Player Loaded : ^9[^7"..time.."^9] ^9(^5"..GetPlayerName(src).." ^9| ^5"..pName.." ^9| ^5"..Player.PlayerData.citizenid.."^9) ^2has succesfully loaded!^7")
        QBCore.Commands.Refresh(src)
        loadHouseData()
        TriggerClientEvent('apartments:client:setupSpawnUI', src, cData)
        TriggerEvent("qb-log:server:CreateLog", "joinleave", "Loaded", "green", {ply = GetPlayerName(src), txt = GetPlayerName(src) .. " ("..(QBCore.Functions.GetIdentifier(src, 'discord') or 'undefined') .."\n"..(QBCore.Functions.GetIdentifier(src, 'license') or 'undefined') .."\n" ..cData.citizenid..") loaded.."})
	end
end)
