...
function createSafe(combination, nSound)
	local res
	...
	elseif _SafeCrackingStates == "Cracking" then
		...
		if not incorrectMovement then
			...
			if correctMovement then
				...
				if pinUnlocked then
					if not noSound then PlaySoundFrontend(0,"TUMBLER_PIN_FALL","SAFE_CRACK_SOUNDSET",true) end
					_onSpot = true
				end
			end
...
function EndMiniGame(safeUnlocked)
    if safeUnlocked then    
        ...
        TriggerEvent('SafeCracker:EndMinigame', safeUnlocked)
    else
