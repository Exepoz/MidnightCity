...
CreateThread(function()
	for k, v in pairs(Config.Locations) do
		if v.garage then
			...
			Targets["WWGarage: "..k] =
			exports['qb-target']:AddBoxZone("WWGarage: "..k, vec3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="WWGarage: "..k, heading = out.w+180.0, debugPoly=Config.Debug, minZ=(out.z-1.03)-0.1, maxZ=out.z-1.03+1.3 },
				{ options = { { event = "jixel-whitewidow:client:Garage:Menu", icon = "fas fa-clipboard", label = Loc[Config.Lan].target["job_veh"], job = v.job, spawncoords = v.garage.spawn, list = v.garage.list }, },
				distance = 2.0 })
			parking[#parking+1] = makeProp({prop = `prop_parkingpay`, coords = vec4(out.x, out.y, out.z, out.w+180)}, 1, false)
		end
	end
end)

local currentVeh = { out = false, current = nil }
RegisterNetEvent('jixel-whitewidow:client:Garage:Menu', function(data)
	...
	local vehicleMenu = { { icon = "fas fa-car-tunnel", header = Loc[Config.Lan].menu["garage"], isMenuHeader = true } }
	vehicleMenu[#vehicleMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "jixel-whitewidow:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-clipboard-list", header = Loc[Config.Lan].menu["out_veh"],
										txt = "Vehicle: "..GetDisplayNameFromVehicleModel(GetEntityModel(currentVeh.current))..Loc[Config.Lan].menu["plate"].."["..GetVehicleNumberPlateText(currentVeh.current).."]",
										params = { event = "jixel-whitewidow:client:Garage:Blip", }, }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = Loc[Config.Lan].menu["remove_veh"], params = { event = "jixel-whitewidow:client:RemSpawn" } }
	else
		...
		for _, v in pairs(data.list) do
			...
			vehicleMenu[#vehicleMenu+1] = { header = name, params = { event = "jixel-whitewidow:client:SpawnList", args = { spawnName = spawnName, coords = data.spawncoords } } }
		end
	end
    exports['qb-menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent("jixel-whitewidow:client:SpawnList", function(data)
	...
	if oldveh ~= 0 then
		...
		triggerNotify(nil, name..Loc[Config.Lan].error["inway"], "error")
	else
...
RegisterNetEvent("jixel-whitewidow:client:RemSpawn", function(data)
...
RegisterNetEvent("jixel-whitewidow:client:Garage:Blip", function(data)
	triggerNotify(nil, Loc[Config.Lan].success["marked"])
	...
	if not DoesBlipExist(garageBlip) then
		...
		AddTextComponentString(Loc[Config.Lan].mark["veh_mark"])
