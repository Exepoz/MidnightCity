...
startMiniGame = (data) => {
  ...
  CONTROL_BAR.style.width = "140px";
  CONTROL_BAR.style.height = "140px";
  CONTROL_BAR.style.fontSize = '2.5rem';
  CONTROL_BAR.style.alignItems = 'center';
  CONTROL_BAR.style.fontWeight = '500';
  CONTROL_BAR.style.fontFamily = "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif";
  CONTROL_BAR.style. color = '#ffffff';
  CONTROL_BAR.id = "controls";
  CONTROL_BAR.innerText = "[E]";
  AREA_WIDTH = data?.width || 15;
  GAME_BODY.appendChild(LOAD_BAR);
  GAME_BODY.appendChild(CONTROL_BAR);
  ...
  const updateCanvas = () => {
    ...
    if (isRadianInInterval(lineArea, areaStart, areaEnd)) {
      loadBar.value < 100
        ...
        : (loadBar.value = 100);
      canvasDraw.newArea(`rgb(${imgColor})`, areaStart, areaEnd, 250, 30, 30);
      canvasDraw.newArea(
        `rgb(0,150,0)`,
        (Math.PI * lineS) / 180,
  ...
  const endGame = async () => {
    sendEndGame(data, loadBar.value)
...
const sendEndGame = (data, value) => {
  loadBar.set(DEFAULT_START_BAR_VALUE);
  LOAD_BAR.remove();
  post("endGame", value);
  value >= (data?.halfSuccessMin || 100)
