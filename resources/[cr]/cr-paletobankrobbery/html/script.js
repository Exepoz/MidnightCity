...
Padlock.Close = function() {
    ...
    $.post('https://cr-paletobankrobbery/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://cr-paletobankrobbery/PadLockClose');
    // if (data.error != null) {
    //     $.post('https://cr-paletobankrobbery/CombinationFail');
    // }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://cr-paletobankrobbery/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://cr-paletobankrobbery/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://cr-paletobankrobbery/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            if (CurrentType == "keypad") {
                ...
                $.post('https://cr-paletobankrobbery/exit');
            }
        }
    };
}); //docready
...
function outOfPins() {
    gameOver = true;
    $.post('https://cr-paletobankrobbery/fail');
    setTimeout(function(){
        reset()
    }, 250)
}
...
function unlock() {
    gameOver = true;
    $.post('https://cr-paletobankrobbery/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
