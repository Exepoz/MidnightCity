...
Citizen.CreateThread(function()
    while GetResourceState('mdn-nighttime') ~= 'started' do
        Wait(0)
    end
    Midnight = exports['mdn-nighttime']:GetMidnightCore()
end)

RegisterCommand('setFoodBuff', function(source, args)
    LocalPlayer.state:set('foodBuff', args[1] or nil, true)
    if args[1] == 'progress' then TriggerEvent("progressbar:client:toggleFaster", true)
    else TriggerEvent("progressbar:client:toggleFaster", false) end
end)

RegisterCommand('getFoodBuff', function(source, args)
    print(LocalPlayer.state.foodBuff)
end)
...
local injectorUsed = -1*(10*60000)
RegisterNetEvent('jim-consumables:Consume', function(itemName, item)
    ...
    local action = Consumables[itemName].action or 0
    local hasUse = false
    if type(action) == "function" then hasUse = true end
    ...
    local hasBuff = item.info.highQuality and item.info.quality >= 97 or false
    local buffJob = item.info.buffJob or nil
	...
	if not cancelled then
        ...
        if stats then
            if stats.hunger then TriggerServerEvent("jim-consumables:server:addNeed", QBCore.Functions.GetPlayerData().metadata["hunger"] + (stats.hunger * (item.info.quality/100)), "hunger") end
        ...
        elseif type == 'injector' then
            injectorUsed = GetGameTimer()
        end
        if stats then
