...
CreateThread(function()
	...
	for _, v in pairs(polelist) do i += 1
		...
		exports['qb-target']:AddBoxZone("UniDance"..i, vec3(v.coords.x, v.coords.y, v.coords.z-1.5), 0.6, 0.6, { name="UniDance"..i, heading = v.coords.w, debugPoly = Config.Debug, minZ = v.minZ, maxZ = v.maxZ, },
			{ options = {
				{ event = "jim-vanillaunicorn:Dance", icon = "fas fa-chair", label = Loc[Config.Lan].info["dance"].." "..tostring(1), job = v.job, loc = v.coords, pole = v.pole, dance = 1 },
				{ event = "jim-vanillaunicorn:Dance", icon = "fas fa-chair", label = Loc[Config.Lan].info["dance"].." "..tostring(2), job = v.job, loc = v.coords, pole = v.pole, dance = 2 },
				{ event = "jim-vanillaunicorn:Dance", icon = "fas fa-chair", label = Loc[Config.Lan].info["dance"].." "..tostring(3), job = v.job, loc = v.coords, pole = v.pole, dance = 3 },
			},
				distance = 1.7 })
	end
...
RegisterNetEvent('jim-vanillaunicorn:Dance', function(data)
	...
	if canDance then
		exports['qb-core']:DrawText('[Spacebar] Stop Dancing | Left/Right Arrows to switch dances')
		local currentDance = data.dance
		loadAnimDict('mini@strip_club@pole_dance@pole_dance'..currentDance)
		local scene = NetworkCreateSynchronisedScene(vec3(data.loc.x, data.loc.y, data.loc.z-0.40), vec3(0.0, 0.0, 0.0), 2, false, true, 1065353216, 0, 1.3)
		NetworkAddPedToSynchronisedScene(PlayerPedId(), scene, 'mini@strip_club@pole_dance@pole_dance' .. currentDance, 'pd_dance_0' .. currentDance, 1.5, -4.0, 1, 1, 1148846080, 0)
	...
	while dancing do
		if dancing then
			...
			if IsControlJustReleased(0, 190) then
				if IsEntityPlayingAnim(PlayerPedId(), "mini@strip_club@pole_dance@pole_dance"..currentDance, "pd_dance_0"..currentDance, 3) then
					UnloadAnimDict("mini@strip_club@pole_dance@pole_dance"..currentDance)
					currentDance = currentDance + 1
                    if currentDance > 3 then currentDance = 1 end
                    LoadAnimDict('mini@strip_club@pole_dance@pole_dance'..currentDance)
                    local scene = NetworkCreateSynchronisedScene(vec3(data.loc.x, data.loc.y, data.loc.z-0.40), vec3(0.0, 0.0, 0.0), 2, false, true, 1065353216, 0, 1.3)
                    NetworkAddPedToSynchronisedScene(PlayerPedId(), scene, 'mini@strip_club@pole_dance@pole_dance' .. currentDance, 'pd_dance_0' .. currentDance, 1.5, -4.0, 1, 1, 1148846080, 0)
                    NetworkStartSynchronisedScene(scene)
            ...
            if IsControlJustReleased(0, 189) then
				if IsEntityPlayingAnim(PlayerPedId(), "mini@strip_club@pole_dance@pole_dance"..currentDance, "pd_dance_0"..currentDance, 3) then
					UnloadAnimDict("mini@strip_club@pole_dance@pole_dance"..currentDance)
					currentDance = currentDance - 1
                    if currentDance < 1 then currentDance = 3 end
                    LoadAnimDict('mini@strip_club@pole_dance@pole_dance'..currentDance)
                    local scene = NetworkCreateSynchronisedScene(vec3(data.loc.x, data.loc.y, data.loc.z-0.40), vec3(0.0, 0.0, 0.0), 2, false, true, 1065353216, 0, 1.3)
                    NetworkAddPedToSynchronisedScene(PlayerPedId(), scene, 'mini@strip_club@pole_dance@pole_dance' .. currentDance, 'pd_dance_0' .. currentDance, 1.5, -4.0, 1, 1, 1148846080, 0)
                    NetworkStartSynchronisedScene(scene)
...
for _, v in pairs(Config.Locations) do
	if v.zoneEnable then
		if v.MLO == "gabz" then
			polelist[#polelist+1] = { coords = vec4(104.76, -1294.18, 29.65, 30.0), minZ = 28.00, maxZ = 32.85, pole = 1, job = 'vanilla' }
			polelist[#polelist+1] = { coords = vec4(108.83, -1289.02, 29.65, 30.0), minZ = 28.00, maxZ = 32.85, pole = 2, job = 'vanilla' }
			polelist[#polelist+1] = { coords = vec4(102.22, -1289.87, 29.65, 30.0), minZ = 28.00, maxZ = 32.85, pole = 3, job = 'vanilla' }
		...
		elseif v.MLO == 'fiv3devs' then
			--Upstairs
			polelist[#polelist+1] = { coords = vec4(121.77, -1289.25, 29.26+0.35, 30.0), minZ = 28.06, maxZ = 32.85, pole = 1 }
			polelist[#polelist+1] = { coords = vec4(116.66, -1292.19, 29.26+0.35, 30.0), minZ = 28.00, maxZ = 32.85, pole = 2 }
			polelist[#polelist+1] = { coords = vec4(111.50, -1295.17, 29.26+0.35, 30.0), minZ = 27.30, maxZ = 31.26, pole = 3 }
			-- VIP Rooms
			polelist[#polelist+1] = { coords = vec4(127.195, -1294.079, 21.72+0.35, 30.0), minZ = 20.30, maxZ = 24.26, pole = 4 }
			polelist[#polelist+1] = { coords = vec4(116.336, -1300.352, 21.72+0.35, 30.0), minZ = 20.30, maxZ = 24.26, pole = 5 }
			polelist[#polelist+1] = { coords = vec4(109.549, -1288.595, 21.72+0.35, 30.0), minZ = 20.30, maxZ = 24.26, pole = 6 }
			polelist[#polelist+1] = { coords = vec4(120.391, -1282.339, 21.72+0.35, 30.0), minZ = 20.30, maxZ = 24.26, pole = 7 }
			-- Naughty Rooms
			polelist[#polelist+1] = { coords = vec4(114.499, -1277.792, 21.42+0.35, 30.0), minZ = 20.30, maxZ = 24.26, pole = 8 }
			polelist[#polelist+1] = { coords = vec4(108.674, -1281.162, 21.42+0.35, 30.0), minZ = 20.30, maxZ = 24.26, pole = 9 }
			polelist[#polelist+1] = { coords = vec4(128.152, -1301.439, 21.42+0.35, 30.0), minZ = 20.30, maxZ = 24.26, pole = 10 }
			polelist[#polelist+1] = { coords = vec4(122.327, -1304.808, 21.42+0.35, 30.0), minZ = 20.30, maxZ = 24.26, pole = 11}
			-- Windows
			polelist[#polelist+1] = { coords = vec4(106.7449, -1313.185, 29.84712+0.35, 30.0), minZ = 28.00, maxZ = 33.26, pole = 12}
			polelist[#polelist+1] = { coords = vec4(109.5838,-1311.546, 29.84712+0.35, 30.0), minZ = 28.00, maxZ = 33.26, pole = 13}
			polelist[#polelist+1] = { coords = vec4(112.2671, -1309.997, 29.84712+0.35, 30.0), minZ = 28.00, maxZ = 33.26, pole = 14}
			-- dancerslist[#dancerslist+1] = { coords = vec4(118.48, -1301.7, 29.27-1, 30.0), minZ= 27.68-1, maxZ= 28.49 }
		end
	end
end

-- dancerslist = tables to dance on
-- poles = poles
