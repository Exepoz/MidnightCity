...
local function IsNight()
	return Midnight.Functions.IsNightTime()
end
------------
-- Heist Start
-----------------

RegisterNetEvent('av_houserobbery:client:setupJob', function(house, code, isLeader)
	Leader = isLeader
	heistFinished = false
	hasJob = true
	houseInfo = house
...
RegisterNetEvent('av_houserobbery:client:startJob', function(house)
...
-- Context Menu
lib.registerContext({ id = 'selectHouse', title = 'Chose Robbery Type',
	options = {
			{title = "Low-End", description = 'Lower Security, Cheaper Loot.', icon = 'house', serverEvent = 'av_houserobbery:server:checkRequirements', args = {tier = 'low'}},
			{title = "High-End", description = 'Better Security, Juicier Loot.', icon = 'house', serverEvent = 'av_houserobbery:server:checkRequirements', args = {tier = 'high'}},
		}
	}
)

RegisterNetEvent('av_houserobbery:BurglaryStart', function()
	local options = {}

	if Leader then options[#options+1] = {title = "Finish Job", description = 'End your current contract.', icon = 'house', serverEvent = 'av_houserobbery:server:finishHeist'}
	else options[#options+1] = {title = "Get House", description = 'Get a contract for a house robbery.', icon = 'house', menu = 'selectHouse'}
	end

	options[#options+1] = {title = "Check Bounties", description = 'Look at which items the boss is looking for today.', icon = 'clipboard-list', onSelect = function() loadBounties() end}
	options[#options+1] = {title = "Check Reputation & Rewards", description = 'Take a look at your rep and it\'s benefits.', icon = 'chart-line', onSelect = function() checkLvl() end}

	lib.registerContext({ id = 'houseRobBoss', title = 'The Boss', options = options })
	lib.showContext('houseRobBoss')
end)

-- Spawn Boss ped when closeby
Citizen.CreateThread(function()
	local bossPoint = lib.points.new(vector4(265.00, 2592.64, 45.19, 181.14), 60) --vector4(-591.87, -1766.64, 23.18, 273.99)
	function bossPoint:onEnter()
		if not DoesEntityExist(PedBoss) then
			PedBoss = CreatePedAtCoords('cs_chengsr', vector4(265.00, 2592.64, 45.19, 181.14))
			...
			exports['qb-target']:AddTargetEntity(PedBoss, {options = options, distance = 1.5})
		end
	end
...
function ClosebyHouseLoop()
	...
	function doorPoint:nearby()
		...
		if IsControlJustReleased(0, 47) and not isInside then
			...
			if not QBCore.Functions.HasItem(Config.LockpickName) then QBCore.Functions.Notify(Config.Lang['lockpick'], 'error') return end
			usingLockpick()
		end
	end
end
...
local function InsideHouse()
	if PlayerJob ~= Config.PoliceJobName then
		for k, v in ipairs(items) do
			...
			local key = #housePoints+1
			function p:onEnter()
				if not self.isClosest or self.robbed or carrying then return end
				SetEntityDrawOutline(v.object, true)
				SetEntityDrawOutlineColor(16, 218, 232, 255)
				lib.showTextUI(Config.Lang['search'])
			end
			function p:nearby()
				if not self.isClosest or self.robbed then return end
				if IsControlJustPressed(0,38) and not carrying then
					TriggerServerEvent('av_houserobbery:server:groupSync', 'takenLoot', key, true)
					lib.hideTextUI()
					if v.anim == 'safe' then
						if Config.Rep[currentLvl] and Config.Rep[currentLvl].autoSafe and math.random(100) <= 20 then
							...
							{}, {}, {}, function()
								...
								TriggerServerEvent('av_houserobbery:server:groupSync', 'takenLoot', key, false)
							end)
						else
							...
							if res then
								SetEntityDrawOutline(v.object, false)
								TriggerServerEvent('av_houserobbery:item','safe')
							else TriggerServerEvent('av_houserobbery:server:groupSync', 'takenLoot', key, false) end
						end
					else
...
RegisterNetEvent('av_houserobbery:noiseLvl', function()
	...
	while true do
		if isInside then
			...
			if GetEntitySpeed(ped) > 1.7 then noiseLvl = noiseLvl + (LocalPlayer.state.foodBuff == 'sneaky' and 5 or 10)
			...
			noiseNUI(noiseLvl)
			if noiseLvl > 100 and not caught then
				caught = true
				exports['ps-dispatch']:HouseRobbery()
				TriggerServerEvent('av_houserobbery:server:groupSync', 'caught', true)
				TriggerServerEvent('av_houserobbery:server:playerCaught')
			end
		end
...
RegisterNetEvent('av_houserobbery:anim', function(anim, deleteObj, i, ground)
	...
	if anim == 'tv' then
		...
		TriggerServerEvent('av_houserobbery:server:groupSync', 'deleteObj', i)
		carrying = true
		local ui = false
	...
	elseif anim == 'telescope' then
		...
		while true do
			...
			if IsControlJustReleased(0, 73) then
				...
				exports['qb-target']:AddTargetEntity(telescopeObj, {options = options, distance = 2.0})
				break
			end
			Citizen.Wait(w)
		end
	...
	elseif anim == 'normal' then
		...
		if deleteObj then TriggerServerEvent('av_houserobbery:server:groupSync', 'deleteObj', i) end
	...
	elseif anim == 'laptop' then
		FreezeEntityPosition(ped,true)
		GrabAnim() TriggerServerEvent('av_houserobbery:server:groupSync', 'deleteObj', i)
		...
		while true do
			...
			if IsControlJustReleased(0, 73) then
				...
				exports['qb-target']:AddTargetEntity(object, {options = options, distance = 2.0})
				FreezeEntityPosition(object, false)
				ActivatePhysics(object)
				break
			end
...
function LeaveHouse(d)
	...
	Citizen.Wait(800)
	DoScreenFadeIn(2600)
	if d then TriggerEvent('av_houserobbery:client:resetAll') end
	SendNUIMessage({closeProgress = true})
	Citizen.Wait(1500)
...
local syncEvents = {
	...
	['caught'] = function(bool)
		caught = bool
		MissionInProgress = false
		if isInside then LeaveHouse() end
	end,
	['deleteObj'] = function(i) DeleteEntity(items[i].object) end,

}
RegisterNetEvent('av_houserobbery:client:groupSync', function(sync, ...) syncEvents[sync](...) end)
