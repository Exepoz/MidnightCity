...
AddItem = function(inv, item, amount, metadata)
    ...
    if Inventory:AddItem(inv, item, amount, false, metadata) then
        --for i = 1, amount do
            TriggerClientEvent('inventory:client:ItemBox', inv, QBCore.Shared.Items[item], 'add', amount)
        --end
        return true
    end
    return false
...
RemoveItem = function(inv, item, amount, metadata, slot)
    ...
    if Inventory:RemoveItem(inv, item, amount, slot) then
        -- for i = 1, amount do
            TriggerClientEvent('inventory:client:ItemBox', inv, QBCore.Shared.Items[item], 'remove', amount)
        -- end
        return true
    end
    return false
