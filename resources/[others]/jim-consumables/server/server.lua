...
for k, v in pairs(Config.Consumables) do
	QBCore.Functions.CreateUseableItem(k, function(source, item) TriggerClientEvent('jim-consumables:Consume', source, item.name, item) end)
...
RegisterNetEvent('jim-consumables:server:syncAddItem', function(itemName, data)
	if not Consumables[itemName] then
		QBCore.Functions.CreateUseableItem(itemName, function(source, item) TriggerClientEvent('jim-consumables:Consume', source, itemName, item) end)
